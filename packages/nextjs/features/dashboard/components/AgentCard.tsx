  ...
  return (
    ...
    >
      ...
      <dl className="mt-4 w-full space-y-1.5">
        ...
        <div className="flex items-baseline justify-between gap-3">
          <dt className="text-label text-muted">Balance</dt>
          <dd className="font-mono text-mono-sm font-medium tabular-nums">{formatHbar(agent.balanceHbar)}</dd>
        </div>
      </dl>

      <p className="mt-3 text-caption text-subtle">Last action {agent.lastActionAgo}</p>
...
export function AddAgentCard({ onAdd }: { onAdd: () => void }) {
  return (
    <button
      onClick={onAdd}
      className="flex min-h-[168px] cursor-pointer flex-col items-center justify-center gap-2.5 rounded-lg border-2 border-dashed border-border-strong bg-transparent p-4 text-center shadow-sm transition-all duration-[160ms] hover:-translate-y-0.5 hover:border-brand hover:bg-brand-soft/40 hover:shadow-md motion-reduce:hover:translate-y-0"
    >
...
export function AgentCardSkeleton() {
  return <div className="h-[168px] animate-pulse rounded-lg bg-surface shadow-sm" />;
}
