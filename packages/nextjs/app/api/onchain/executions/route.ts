...
import { listSafeExecutions } from "~~/lib/onchain-data/coverageRepository";
  ...
  try {
    ...
    const result = await listSafeExecutions({
      ...
      filters: {
        ...
        success: optionalBoolean(params, "success"),
        safeTxHash: optional(params, "safeTxHash"),
...
function optionalBoolean(params: URLSearchParams, name: string): boolean | undefined {
  const value = optional(params, name)?.toLowerCase();
  if (value === undefined) return undefined;
