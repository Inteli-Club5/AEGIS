...
import { listCrossChainAgents } from "~~/lib/onchain-data/crossChainAgents";
...
export async function GET(request: Request) {
  try {
    ...
    const result = await listCrossChainAgents({
      hederaClient: getClientOrDeferredFailure(getHederaGraphClient),
      zeroGClient: getClientOrDeferredFailure(getZeroGGraphClient),
      ...
      filters: {
        ...
        tokenId: optional(params, "tokenId") ?? optional(params, "agenticIdTokenId"),
...
function getClientOrDeferredFailure(factory: () => GraphClient): GraphClient {
  try {
    ...
    return { query: async <T>() => Promise.reject(error) as Promise<T> };
  }
}
